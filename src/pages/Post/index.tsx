...
const Post = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          const authorResponse = await api.get<IUser>(
            `/users/${postResponse.data.id_user}`
          );
  ...
  if (error) {
    return (
      <h5 className="text-center pt-48 color-red">
        Houve um erro ao carregar os dados da página, tente recarregar a página
        novamente!
      </h5>
    );
  }
  ...
  if (loading) {
    return (
      <h2 className="text-center small-content-container mt-32">
        Carregando...
      </h2>
    );
  }
  ...
  return (
    <article className="container mt-32 text-container">
      <header>
        <span className="h6 color-gray fw-normal uppercase">
          {postData?.category}
        </span>
        <h1 className="fw-normal h3">{postData?.title}</h1>
        {authorData && (
          <div className="mt-8">
            <h2 className="color-white h6">
              Escrito por {authorData?.name} {authorData?.surname}
            </h2>
            <time className="h6 color-gray">
              {formatDateToPost(postData?.date ?? new Date())} - leitura de{" "}
              {postData?.duration}
            </time>
          </div>
        )}
      ...
      <footer className="row mt-40">
        ...
        {authorData && (
          <div className="col-10 border-black px-16 py-32">
            <div className="flex-start">
              ...
              <div className="ml-32 flex-grow-1">
                <h6 className="fw-normal text-center color-white">
